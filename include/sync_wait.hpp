...
template <concepts::awaitable A,
          typename R = awaitable_traits<A>::await_resume_t>
details::task_with_callback<R> make_sync_wait(A &&awaitable,
...
template <concepts::awaitable A,
          typename R = awaitable_traits<A>::await_resume_t>
  requires std::same_as<void, R> details::task_with_callback<void>
  make_sync_wait(A &&awaitable, std::atomic<bool> &flag) {
    // GCC BUG TRACK : https://gcc.gnu.org/bugzilla/show_bug.cgi?id=99575
    // https://godbolt.org/z/anWWjb4j4
    // co_await std::forward<A>(awaitable)
...
template <concepts::awaitable F> decltype(auto) sync_wait(F &&f) {
