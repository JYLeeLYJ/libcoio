...
template<class T>
class task_with_callback : non_copyable{
public :
    ...
    task_with_callback(std::coroutine_handle<promise_type> handle) : m_handle (handle){
    }
    ...
    task_with_callback& operator=(task_with_callback && othr){
        if(m_handle) this->~task_with_callback();
    ...
    void set_callback(F &&f ) {
        if(m_handle) m_handle.promise().m_callback = std::forward<F>(f); 
    }
