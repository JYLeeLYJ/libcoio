...
namespace details {
static constexpr inline msghdr default_maker(iovec *iov_ptr,
                                             std::size_t iov_len) {
  return {};
}
} // namespace details
...
public:
  ...
  void set_no_delay() {
    int reuse = 1;
    int ret = ::setsockopt(file_descriptor_base::fd, SOL_TCP, TCP_NODELAY,
                           &reuse, sizeof(int));
    if (ret != 0)
      throw make_system_error(errno);
  }
...
public:
  ...
  tcp_sock<Domain> &socket() { return *this; }
};
