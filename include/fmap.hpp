...
template <concepts::awaitable Awaitable, class Fn> struct fmap_awaitable {

  Awaitable awaitable;
  Fn continuation;

  template <class A, class F> struct fmap_awaiter {
    // use lvalue
    ...
    template <class P>
    decltype(auto) await_suspend(std::coroutine_handle<P> handle) noexcept(
        noexcept(awaiter.await_suspend(handle))) {
...
template <class Fn> struct fmap_transform { Fn fn; };

// then
template <class Fn> auto fmap(Fn &&fn) {
...
concept transform_function =
    (std::is_void_v<Arg> && std::invocable<F>) || std::invocable<F, Arg>;

}

template <concepts::awaitable A, class Fn>
// requires concepts::transform_function<Fn ,typename
// awaitable_traits<A>::await_resume_t>
auto fmap(A &&awaitable, Fn &&fn) {
  return fmap_awaitable<A, Fn>{.awaitable = std::forward<A>(awaitable),
...
template <class A, class Fn>
decltype(auto) operator|(A &&awaitable, fmap_transform<Fn> &&then) {
  return fmap(std::forward<A>(awaitable),
              std::forward<decltype(then)>(then).fn);
}
