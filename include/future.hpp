...
public:
  ...
  constexpr auto final_suspend() noexcept { return final_awaiter{}; }
...
template <class T> class future_promise : public future_promise_base {
public:
...
public:
  ...
  T result() && requires std::is_scalar_v<T> { return get(); }

  // note : this rval reference return is safe
  T &&result() && { return std::move(get()); }
...
template <> class future_promise<void> : public future_promise_base {
public:
...
template <concepts::future_value T> class future : non_copyable {
public:
...
public:
  ...
  future(future &&f) noexcept : m_handle(f.m_handle) { f.m_handle = nullptr; }
...
public:
  template <bool use_rvalue> struct awaiter : basic_awaiter {
  ...
  auto operator co_await() const &&noexcept { return awaiter<true>{m_handle}; }

  auto operator co_await() const &noexcept { return awaiter<false>{m_handle}; }
