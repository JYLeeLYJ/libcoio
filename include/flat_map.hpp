...
using flatmap_deduce_t =
    typename awaitable_traits<std::invoke_result_t<Fn, R>>::await_result_t;

template <class Fn>
using flatmap_deduce_void_t =
    typename awaitable_traits<std::invoke_result_t<Fn>>::await_result_t;
...
auto flat_map_impl(Awaitable awaitable, Fn &&fn)
    -> future<details::flatmap_deduce_void_t<Fn>> {
...
template <concepts::awaitable Awaitable, class Fn,
          class R = typename awaitable_traits<Awaitable>::await_resume_t>
  requires concepts::binder_function<Fn, R>
auto flat_map_impl(Awaitable awaitable, Fn &&fn)
    -> future<details::flatmap_deduce_t<Fn, R>> {
  // GCC BUG TRACK : https://gcc.gnu.org/bugzilla/show_bug.cgi?id=99575
  // https://godbolt.org/z/anWWjb4j4
  // co_return co_await std::forward<Fn>(fn)(co_await
  // std::forward<Awaitable>(awaitable));
  co_return co_await std::forward<Fn>(fn)(
      co_await reinterpret_cast<Awaitable &&>(awaitable));
}
} // namespace details

template <class Fn> struct flat_map_transform { Fn fn; };

template <class Fn> auto flat_map(Fn &&fn) {
...
auto flat_map(Awaitable &&a, Fn &&fn) {
  return details::flat_map_impl<Awaitable>(std::forward<Awaitable>(a),
                                           std::forward<Fn>(fn));
}
