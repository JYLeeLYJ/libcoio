...
TEST(test_future, test_result_lifetime) {
  ...
  // destory result when future destory
  {
    auto ft = f();
    EXPECT_EQ(cnt, 0);
    [[maybe_unused]] auto &result = sync_wait(ft);
    EXPECT_EQ(cnt, 1);
  }
  EXPECT_EQ(cnt, 0);

  auto f2 = [](count c) -> future<count> { co_return c; };
  ...
  {
    [[maybe_unused]] auto &&result = sync_wait(f2(count{}));
...
TEST(test_future, test_return_convertiable) {
  ...
  [[maybe_unused]] auto f = []() -> future<foo> { co_return 1; };
  auto f2 = []() -> future<int> { co_return foo2{}; };
...
TEST(test_future, test_return_reference) {
  int value{3};
  auto f = [&]() -> future<ref<int>> { co_return ref{value}; };
...
TEST(test_future, test_move_and_copy_counting) {
  ...
  [[maybe_unused]] auto &result1 =
      sync_wait(future1); // return lvalue ref cause 'future1' is lvalue.
  EXPECT_EQ(copyn, 0);
  EXPECT_EQ(moven, 1);
}
