...
TEST(test_fmap , test_just){
    ...
    EXPECT_EQ(sync_wait(just<void>{} | receiver | fmap(std::negate<int>{})) , -114514);
}
...
TEST(test_fmap , test_lifetime){
    ...
    {   // rvalue will be move into a
        is_destory = false;
        auto a = awaiter{&is_destory} | flat_map([]{return std::suspend_never{}; });
        EXPECT_FALSE(is_destory);
    }
...
// template<class T>
// future_t foo(T t){
//     co_await std::forward<T>(t);    //will invoke move constructor here , no matter T = awaiter& or awaiter
// }
