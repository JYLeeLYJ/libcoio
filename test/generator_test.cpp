...
TEST(test_generator, test_reference_type) {
  ...
  auto g3 = []() -> generator<std::string> { co_yield "fffff"s; }();
  auto it3 = g3.begin();
...
namespace {
template <std::invocable<> F> struct on_exit_t {
  F f;
...
TEST(test_generator, test_with_stdranges) {
  // requires ranges
  std::ranges::range auto g = []() -> generator<uint> {
    for (uint i = 0; true; ++i) {
      co_yield i;
    }
  }();
  ...
  [[maybe_unused]] std::input_or_output_iterator auto beg = g.begin();

  auto even = [](uint i) { return !(i & 1); };

  for (uint v = 0;
       auto i : g | std::views::filter(even) | std::views::take(10)) {
    ASSERT_EQ(i, v);
    v += 2;
  }
}